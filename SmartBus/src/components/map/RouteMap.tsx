...
export function RouteMap({
  ...
  const getStopTypeText = (type: string) => {
    switch (type) {
      case 'school': return 'Trường học';
      case 'regular': return 'Trạm thường';
      case 'home': return 'Khu dân cư';
  ...
  const getStopTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'regular': return <MapPin className="w-4 h-4" />;
      ...
      <div className={`space-y-4 ${isMobile ? 'order-2' : ''}`}>
        ...
        {selectedRoute && showStops && (
          <Card>
            ...
            <CardContent className={`${isMobile ? 'p-2' : ''} space-y-2 max-h-[300px] overflow-y-auto`}>
              {selectedRoute.stops.map((stop, index) => (
                <div
                  key={stop.id}
                  className={`p-3 border rounded-lg cursor-pointer transition-colors hover:bg-gray-50 ${
                    selectedStop?.id === stop.id ? 'border-blue-500 bg-blue-50' : ''
                  }`}
...
// Component hiển thị thống kê tuyến đường
export function RouteStats({ route }: { route: RouteData }) {
  const isMobile = useIsMobile();
  
  const totalStudents = route.stops.reduce((sum, stop) => sum + stop.studentsCount, 0);
  const schoolStops = route.stops.filter(stop => stop.type === 'school').length;
  const regularStops = route.stops.filter(stop => stop.type === 'regular').length;
    ...
    <Card>
      ...
      <CardContent className={`${isMobile ? 'p-3' : ''} space-y-4`}>
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="text-center p-3 bg-gray-50 rounded-lg">
            <div className="font-medium text-lg">{route.estimatedDuration} phút</div>
