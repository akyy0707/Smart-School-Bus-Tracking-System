...
type DriverStatusType = 'active' | 'break' | 'incident' | 'offline';

export function DriverApp({ onBack }: DriverAppProps) {
  const [activeTab, setActiveTab] = useState('dashboard');
  const [driverStatus, setDriverStatus] = useState<DriverStatusType>('active');
  ...
  const getStatusColor = (status: DriverStatusType) => {
    switch (status) {
  ...
  const getStatusText = (status: DriverStatusType) => {
    switch (status) {
            ...
            <div className="flex items-center gap-2 md:gap-4">
              {!isMobile && (
                <div className="flex items-center gap-2">
                  <div className={`w-3 h-3 rounded-full ${getStatusColor(driverStatus)}`} />
                  <span className="text-sm">{getStatusText(driverStatus)}</span>
                </div>
              )}
              
              <Avatar className={isMobile ? 'w-8 h-8' : ''}>
                <AvatarFallback className={isMobile ? 'text-sm' : ''}>{driverInfo.avatar}</AvatarFallback>
              </Avatar>
      ...
      <div className="border-b border-gray-200 bg-white">
        <div className={isMobile ? 'px-2' : 'px-4'}>
          <AnimatedTabs value={activeTab} onValueChange={setActiveTab}>
            <AnimatedTabsList className={`${isMobile ? 'h-16 w-full justify-start' : 'h-12'} bg-transparent border-0 ${isMobile ? 'overflow-x-auto' : ''}`}>
              <AnimatedTabsTrigger value="dashboard" className={`${isMobile ? 'flex-col gap-1 min-w-[60px] text-xs' : 'gap-2'}`}>
                <User className="w-4 h-4" />
                {isMobile ? 'Tổng quan' : 'Tổng quan'}
              </AnimatedTabsTrigger>
              <AnimatedTabsTrigger value="schedule" className={`${isMobile ? 'flex-col gap-1 min-w-[60px] text-xs' : 'gap-2'}`}>
                <Calendar className="w-4 h-4" />
                {isMobile ? 'Lịch' : 'Lịch trình'}
              </AnimatedTabsTrigger>
              <AnimatedTabsTrigger value="students" className={`${isMobile ? 'flex-col gap-1 min-w-[60px] text-xs' : 'gap-2'}`}>
                <Users className="w-4 h-4" />
                {isMobile ? 'H.sinh' : 'Học sinh'}
              </AnimatedTabsTrigger>
              <AnimatedTabsTrigger value="gps" className={`${isMobile ? 'flex-col gap-1 min-w-[60px] text-xs' : 'gap-2'}`}>
                <Navigation className="w-4 h-4" />
                {isMobile ? 'GPS' : 'Theo dõi GPS'}
              </AnimatedTabsTrigger>
              <AnimatedTabsTrigger value="notifications" className={`${isMobile ? 'flex-col gap-1 min-w-[60px] text-xs' : 'gap-2'}`}>
                <Bell className="w-4 h-4" />
                {isMobile ? 'T.báo' : 'Thông báo'}
              </AnimatedTabsTrigger>
              <AnimatedTabsTrigger value="status" className={`${isMobile ? 'flex-col gap-1 min-w-[60px] text-xs' : 'gap-2'}`}>
