...
export function DriverStudents({ driverId }: DriverStudentsProps) {
  ...
  const updateAttendance = (studentId: string, attendance: 'present' | 'absent') => {
  ...
  const getAttendanceBadge = (attendance: 'present' | 'absent' | 'pending') => {
    switch (attendance) {
      case 'present':
  ...
  const getAttendanceIcon = (attendance: 'present' | 'absent' | 'pending') => {
    switch (attendance) {
      case 'present':
  ...
  const filteredStudents = students.filter(student => {
    ...
    const matchesFilter = attendanceFilter === 'all' || student.attendance === attendanceFilter;
                            ...
                            <div className="space-y-4">
                              ...
                              <div className="space-y-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-sm text-muted-foreground">
                                    {selectedStudent.pickupLocation} ({selectedStudent.pickupTime})
                                  </p>
                                </div>
                                <div>
                                  <p className="font-medium">Điểm trả:</p>
                                  <p className="text-sm text-muted-foreground">
                                    {selectedStudent.dropoffLocation} ({selectedStudent.dropoffTime})
                                  </p>
                                </div>
                              ...
                              {selectedStudent.attendance === 'pending' && (
                                <div className="flex gap-2 pt-4">
                                  <Button
                                    ...
                                    onClick={() => {
                                      updateAttendance(selectedStudent.id, 'present');
                                  ...
                                  <Button
                                    ...
                                    onClick={() => {
                                      updateAttendance(selectedStudent.id, 'absent');
                                      setSelectedStudent(null);
                                    }}
                                  >
