...
export function DriverDashboard({ driverInfo, currentVehicle, driverStatus }: DriverDashboardProps) {
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <span>{new Date(driverInfo.birthDate).toLocaleDateString('vi-VN')}</span>
              </div>
            </div>
          </div>
        </CardContent>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <Badge className="bg-blue-100 text-blue-800">{currentVehicle.licensePlate}</Badge>
              </div>
              <div className="flex items-center gap-2">
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="font-medium">{todayStats.completedTrips}/{todayStats.totalTrips}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              <div className={`w-3 h-3 rounded-full ${
                ...
                driverStatus === 'break' ? 'bg-yellow-500' : 
                driverStatus === 'incident' ? 'bg-red-500' : 'bg-gray-500'
              }`} />
              <div>
                <p className="text-sm text-muted-foreground">Trạng thái</p>
                <p className="font-medium">
                  {driverStatus === 'active' ? 'Hoạt động' :
                   driverStatus === 'break' ? 'Nghỉ' :
                   driverStatus === 'incident' ? 'Sự cố' : 'Ngoại tuyến'}
                </p>
              </div>
            </div>
          </CardContent>
