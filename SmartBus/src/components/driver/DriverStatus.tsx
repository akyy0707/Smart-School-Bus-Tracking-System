...
interface DriverStatusProps {
  currentStatus: 'active' | 'break' | 'incident' | 'offline';
  onStatusChange: (status: 'active' | 'break' | 'incident' | 'offline') => void;
}

export function DriverStatus({ currentStatus, onStatusChange }: DriverStatusProps) {
  ...
  const handleStatusChange = (newStatus: typeof currentStatus) => {
    const statusInfo = statusOptions.find(option => option.value === newStatus);
    ...
    <div className="space-y-6">
          ...
          <div className={`p-4 rounded-lg border-2 ${currentStatusInfo?.color}`}>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm opacity-75">{currentStatusInfo?.description}</p>
              </div>
            </div>
          </div>
        </CardContent>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {statusOptions.map((option) => (
              ...
              >
                ...
                >
                  {currentStatus === option.value ? 'Đang áp dụng' : 'Chuyển sang'}
                </Button>
              </div>
            ))}
          </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Nếu gặp sự cố hoặc tình huống bất thường, vui lòng mô tả chi tiết để chúng tôi hỗ trợ kịp thời.
          </p>
        ...
        <CardContent>
          <div className="space-y-3">
            {recentStatusChanges.map((change, index) => {
              const statusInfo = statusOptions.find(opt => opt.value === change.status);
              return (
                <div key={index} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                  {statusInfo?.icon}
                  <div className="flex-1">
                    <p className="font-medium">{statusInfo?.label}</p>
                    <p className="text-sm text-muted-foreground">{change.note}</p>
                  </div>
