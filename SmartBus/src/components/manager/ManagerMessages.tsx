...
export function ManagerMessages() {
  const [selectedRecipient, setSelectedRecipient] = useState<'all_drivers' | 'all_parents' | 'specific' | ''>('');
  ...
  const recentMessages = [
    ...
    },
    {
      ...
      type: 'reminder',
      content: 'Xe buýt sẽ đến điểm đón trong 10 phút. Vui lòng chuẩn bị sẵn sàng.',
  ...
  const getRecipientIcon = (type: string) => {
    switch (type) {
      case 'drivers': return <Users className="w-4 h-4 text-green-600" />;
      case 'parents': return <Users className="w-4 h-4 text-purple-600" />;
        ...
        <CardContent>
          <div className="grid md:grid-cols-3 gap-4">
            <Button 
              ...
              onClick={() => setMessage('Xe buýt sẽ đến điểm đón trong 10 phút. Vui lòng chuẩn bị sẵn sàng.')}
            >
              <div>
                <p className="font-medium">Thông báo đến điểm</p>
                <p className="text-sm text-muted-foreground">Gửi cho phụ huynh</p>
              </div>
            </Button>
            ...
            <Button 
              variant="outline" 
              className="h-auto p-4 text-left"
              onClick={() => setMessage('Vui lòng kiểm tra tình trạng xe và báo cáo trước khi bắt đầu ca làm việc.')}
            >
              <div>
            ...
            <Button 
              variant="outline" 
              className="h-auto p-4 text-left"
              onClick={() => setMessage('Do thời tiết xấu, lịch trình có thể bị thay đổi. Chúng tôi sẽ thông báo cập nhật sớm nhất.')}
            >
              <div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <span className={`text-xs px-2 py-1 rounded ${getMessageTypeColor(msg.type)}`}>
                          {msg.type === 'urgent' ? 'Khẩn cấp' :
                           msg.type === 'reminder' ? 'Nhắc nhở' : 'Thông tin'}
                        </span>
