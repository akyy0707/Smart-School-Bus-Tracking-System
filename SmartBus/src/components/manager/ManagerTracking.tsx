...
export function ManagerTracking() {
  const [vehicles, setVehicles] = useState([
    ...
    },
    {
      ...
      route: 'Tuyến 2',
      position: { lat: 10.8350, lng: 106.6400 },
      speed: 28,
      status: 'active' as const,
      lastUpdate: new Date().toLocaleTimeString('vi-VN'),
      studentsOnBoard: 38,
    ...
    },
    {
      ...
      route: 'Tuyến 3',
      position: { lat: 10.8100, lng: 106.6500 },
      speed: 0,
      status: 'stopped' as const,
      lastUpdate: new Date(Date.now() - 5 * 60 * 1000).toLocaleTimeString('vi-VN'),
      studentsOnBoard: 45,
      totalCapacity: 45,
      fuel: 80,
      temperature: 27,
  ...
  const mapMarkers = vehicles.map(vehicle => ({
    ...
    type: "bus" as const,
    status: vehicle.status === 'active' ? 'active' : vehicle.status === 'stopped' ? 'warning' : 'danger' as const,
    info: `
  ...
  const mapRoutes = [
    {
      ...
      path: [
        ...
        { lat: 10.8300, lng: 106.6350 },
        { lat: 10.8350, lng: 106.6400 },
        { lat: 10.8400, lng: 106.6450 }
      ]
    },
    {
      id: 'route2', 
      ...
      path: [
        { lat: 10.8350, lng: 106.6400 },
        { lat: 10.8380, lng: 106.6430 },
        { lat: 10.8410, lng: 106.6460 },
        { lat: 10.8440, lng: 106.6490 }
      ]
    },
    {
      id: 'route3',
      ...
      color: '#f59e0b',
      path: [
        { lat: 10.8100, lng: 106.6500 },
        { lat: 10.8150, lng: 106.6550 },
        { lat: 10.8200, lng: 106.6600 },
        { lat: 10.8250, lng: 106.6650 }
      ]
    }
  ];
    ...
    const interval = setInterval(() => {
      setVehicles(prev => prev.map(vehicle => ({
        ...
        speed: vehicle.status === 'active' 
          ? Math.max(0, Math.min(60, vehicle.speed + (Math.random() - 0.5) * 10))
          : 0,
        lastUpdate: new Date().toLocaleTimeString('vi-VN')
      })));
    }, 3000);
          ...
          <div className={`flex items-center ${isMobile ? 'flex-col gap-3' : 'justify-between'}`}>
            ...
            <div className="flex gap-2">
              <Select value={viewMode} onValueChange={(value: 'map' | 'list') => setViewMode(value)}>
                <SelectTrigger className={`w-auto ${isMobile ? 'text-xs' : ''}`}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          <CardContent>
            <div className="space-y-4">
              {vehicles.map((vehicle) => (
                <div 
                  key={vehicle.id}
                  className={`p-4 border rounded-lg ${selectedVehicleId === vehicle.id ? 'border-blue-500 bg-blue-50' : 'border-gray-200'} cursor-pointer transition-colors`}
                  onClick={() => setSelectedVehicleId(selectedVehicleId === vehicle.id ? '' : vehicle.id)}
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs text-muted-foreground">Cập nhật: {vehicle.lastUpdate}</p>
                    </div>
                  </div>
                  ...
                  {selectedVehicleId === vehicle.id && (
                    <div className="mt-4 pt-4 border-t grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <p className="font-medium">{vehicle.studentsOnBoard}/{vehicle.totalCapacity}</p>
                      </div>
                      <div>
                      ...
                      <div className="col-span-2">
                        ...
                        <p className="font-medium">{vehicle.estimatedArrival}</p>
                      </div>
                    </div>
                  )}
                </div>
