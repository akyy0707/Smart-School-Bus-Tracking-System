...
export function ManagerVehicles() {
  ...
  const handleAddVehicle = () => {
    if (!newVehicle.licensePlate || !newVehicle.brand || !newVehicle.model || !newVehicle.seats) {
    ...
    setNewVehicle({ licensePlate: '', brand: '', model: '', seats: '', status: 'active', condition: 'good' });
              ...
              {filteredVehicles.map((vehicle) => (
                <TableRow 
                  ...
                  className={`cursor-pointer transition-colors ${
                    selectedVehicleId === vehicle.id ? 'bg-blue-50 border-l-4 border-blue-500' : ''
                  }`}
                  ...
                  <TableCell>
                    {vehicle.assignedDriver && vehicle.currentRoute ? (
                      <div className="space-y-1">
                        ...
                        <p className="text-sm text-muted-foreground">{vehicle.currentRoute}</p>
                      </div>
                    ) : (
                  ...
                  <TableCell>
                    <div className="space-y-1">
                      <div className="text-sm">
                        <span className="font-medium">{vehicle.totalTrips.toLocaleString()}</span> chuyến
                      </div>
                  ...
                  <TableCell>
                    <div className="space-y-1">
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span>Cuối: {new Date(vehicle.lastMaintenance).toLocaleDateString('vi-VN')}</span>
                      </div>
                      <div className="flex items-center gap-2 text-sm">
                        <Clock className="w-3 h-3" />
                        <span className={isMaintenanceDue(vehicle.nextMaintenance) ? 'text-red-600 font-medium' : ''}>
                      ...
                      {isMaintenanceDue(vehicle.nextMaintenance) && (
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-xs text-red-600">Sắp đến hạn!</span>
                        </div>
                      )}
                    </div>
                  </TableCell>
        ...
        <CardContent>
          <div className="space-y-3">
            {vehicles
              ...
              .map(vehicle => (
                <div key={vehicle.id} className="flex items-center gap-4 p-4 bg-red-50 border border-red-200 rounded-lg">
                  <AlertTriangle className="w-5 h-5 text-red-600" />
                  <div className="flex-1">
                    <p className="font-medium">Xe {vehicle.licensePlate} sắp đến hạn bảo trì</p>
                    <p className="text-sm text-muted-foreground">
                      Ngày bảo trì tiếp theo: {new Date(vehicle.nextMaintenance).toLocaleDateString('vi-VN')}
                    </p>
                  </div>
            ...
            {vehicles.filter(v => isMaintenanceDue(v.nextMaintenance)).length === 0 && (
              <div className="text-center py-8">
                <CheckCircle className="w-12 h-12 mx-auto mb-4 text-green-600" />
                <h3 className="font-medium mb-2">Tất cả xe đều trong tình trạng tốt</h3>
                <p className="text-muted-foreground">Không có xe nào cần bảo trì trong thời gian tới.</p>
              </div>
            )}
          </div>
        </CardContent>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setNewVehicle(prev => ({ ...prev, seats: e.target.value }))}
                placeholder="VD: 45"
              />
            </div>
            ...
            <div>
              ...
              <Select value={newVehicle.condition} onValueChange={(value) => setNewVehicle(prev => ({ ...prev, condition: value }))}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setSelectedVehicle(prev => ({ ...prev, seats: parseInt(e.target.value) }))}
                />
              </div>
              ...
              <div>
                ...
                <Select value={selectedVehicle.condition} onValueChange={(value) => setSelectedVehicle(prev => ({ ...prev, condition: value }))}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
        ...
        <DialogContent className="max-w-2xl">
          ...
          {selectedVehicle && (
            <div className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span>{selectedVehicle.assignedDriver || 'Chưa phân công'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Tuyến đường:</span>
                      <span>{selectedVehicle.currentRoute || 'Chưa phân công'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Tổng chuyến:</span>
                      <span>{selectedVehicle.totalTrips?.toLocaleString()} chuyến</span>
                    </div>
                    <div className="flex justify-between">
              ...
              <div className="space-y-4">
                <h3 className="font-medium">Thông tin bảo trì</h3>
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    <span className="text-muted-foreground">Bảo trì gần nhất:</span>
                    <p className="font-medium">{new Date(selectedVehicle.lastMaintenance).toLocaleDateString('vi-VN')}</p>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Bảo trì tiếp theo:</span>
                    <p className="font-medium">{new Date(selectedVehicle.nextMaintenance).toLocaleDateString('vi-VN')}</p>
                  </div>
                </div>
              </div>
